...
void app_main() {
  ...
  uart_init();
  ws_server_start();
  xTaskCreate(&server_task,"server_task",6000,NULL,9,NULL);
  xTaskCreate(&server_handle_task,"server_handle_task",10000,NULL,6,NULL);
  xTaskCreate(&uart_handler,"count_task",3000,NULL,12,NULL);
